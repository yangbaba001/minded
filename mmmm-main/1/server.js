...
async function checkVisitorSecurity(req) {
    ...
    if (ipCached && (Date.now() - ipCached.timestamp < IP_CACHE_DURATION)) {
        ...
        try {
            ...
            // Use default data if lookup fails
        }
    }
    ...
    const hostingProviders = [
        ...
        'hosting', 'datacenter', 'data center', 'dedicated', 'server', 'vps'
    ];

    const vpnKeywords = [
        'vpn', 'proxy', 'hide', 'anonymous', 'privacy', 'secure', 'tunnel',
    ...
    const isNonResidential = isHosting || isVPN || isSuspiciousASN;
    
    locationData.ipType = isVPN ? 'VPN/Proxy' : isHosting ? 'Data Center' : 
                          isSuspiciousASN ? 'Suspicious Network' : 'Residential';
    locationData.isVPN = isVPN;
    locationData.isDataCenter = isHosting;
